...
def training_prediction_split(
    new_season="2020", data_folder="data/", predictor_set="model_1"
):
    """
    ...
    for acting_category in [
        ...
        if "Supporting" in acting_category:
            ...
            predictors = lead_acting_predictors
        X_pred[acting_category] = df_acting_new.loc[
            df_acting_new["Category"] == acting_category, predictors
        ]
...
def run(new_season):
    ...
    for model in ["logit", "rf"]:
        ...
        winners.to_csv(
            f"results/winner_predictions_{model}_{new_season}.csv", index=False
        )
        prediction_df.to_csv(
            f"results/all_predictions_{model}_{new_season}.csv", index=False
        )
