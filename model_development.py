...
def training_validation_split(
    year_selection, data_folder="data/", predictor_set="model_0"
):
    """
    ...
    variable_selection = pd.read_excel(
        "predictor_selection.xlsx", sheet_name=predictor_set
    )
    picture_predictors = variable_selection["Variable"][
        variable_selection["Picture"].fillna(0) == 1
    ]
    director_predictors = variable_selection["Variable"][
        variable_selection["Director"].fillna(0) == 1
    ]
    lead_acting_predictors = variable_selection["Variable"][
        variable_selection["Lead Acting"].fillna(0) == 1
    ]
    ...
    X_val = dict()
    y_val = dict()
    X_train["Picture"] = df_picture.loc[
        ~df_picture["Year"].isin(year_selection), picture_predictors
    ]
    y_train["Picture"] = df_picture.loc[
        ~df_picture["Year"].isin(year_selection), "Winner"
    ]
    X_train["Director"] = df_director.loc[
        ~df_director["Year"].isin(year_selection), director_predictors
    ]
    y_train["Director"] = df_director.loc[
        ~df_director["Year"].isin(year_selection), "Winner"
    ]
    X_val["Picture"] = df_picture.loc[
        df_picture["Year"].isin(year_selection), picture_predictors
    ]
    y_val["Picture"] = df_picture.loc[
        df_picture["Year"].isin(year_selection), ["Winner", "Year", "Category"]
    ]
    X_val["Director"] = df_director.loc[
        df_director["Year"].isin(year_selection), director_predictors
    ]
    y_val["Director"] = df_director.loc[
        df_director["Year"].isin(year_selection), ["Winner", "Year", "Category"]
    ]
    for acting_category in [
        "Actor",
        "Actress",
        "Supporting Actor",
        "Supporting Actress",
        ...
        X_train[acting_category] = df_acting.loc[
            (~df_acting["Year"].isin(year_selection))
            & (df_acting["Category"] == acting_category),
            predictors,
        ]
        y_train[acting_category] = df_acting.loc[
            (~df_acting["Year"].isin(year_selection))
            & (df_acting["Category"] == acting_category),
            "Winner",
        ]
        X_val[acting_category] = df_acting.loc[
            (df_acting["Year"].isin(year_selection))
            & (df_acting["Category"] == acting_category),
            predictors,
        ]
        y_val[acting_category] = df_acting.loc[
            (df_acting["Year"].isin(year_selection))
            & (df_acting["Category"] == acting_category),
...
def model_0_validation(model_type, year_selections, k=3, predictor_set="model_0"):
    ...
    for i in range(k):
        ...
        X_train, y_train, X_val, y_val = training_validation_split(
            year_selection, predictor_set=predictor_set
        )
        ...
        for category in categories:
            ...
            for year in val_years.unique():
                ...
                class_df.loc[class_df["Year"] == year, "Classification"] = (
                    class_df.loc[class_df["Year"] == year, "Prob"] == maxprob
                ) * 1
            ...
            out_table = out_table.append(
                {
                    "Category": category,
                    "AUC": roc_auc_score(
                        class_df["Winner"], class_df["Classification"]
                    ),
                    "TPR": recall_score(class_df["Winner"], class_df["Classification"]),
                },
                ignore_index=True,
            )
...
def model_1_validation(model_type, year_selections, k=3, predictor_set="model_1"):
    ...
    for i in range(k):
        ...
        for model_cat, oscar_cats in categories.items():
            ...
            for cat in val_cats:
                for year in val_years.unique():
                    maxprob = class_df.loc[
                        (class_df["Year"] == year) & (class_df["Category"] == cat),
                        "Prob",
                    ].max()
                    class_df.loc[
                        (class_df["Year"] == year) & (class_df["Category"] == cat),
            ...
            for cat in oscar_cats:
                class_df.loc[class_df["Category"] == cat, "Winner"] = y_val[cat][
                    "Winner"
                ]
                ...
                out_table = out_table.append(
                    {
                        "Category": cat,
                        "AUC": roc_auc_score(
                            class_df.loc[class_df["Category"] == cat, "Winner"],
...
def model_3_validation(
    model_type,
    year_selections,
    k=3,
    predictor_set="model_0",
    solver="lbfgs",
    tol=1e-4,
    Cs=10,
):
        ...
        for category in categories:
            ...
            if model_type.lower() in ["logit", "logistic regression"]:
                model = LogisticRegressionCV(
                    max_iter=5000, solver=solver, tol=tol, Cs=Cs
                )
            elif model_type.lower() in ["rf", "random forest"]:
                model = RandomForestClassifier(n_estimators=250)
            else:
...
all_selected_years = np.random.choice(
    years, p=propensity, size=5 * round(num_seasons * 0.15), replace=False
)
...
model_0_logit_out = model_0_validation(
    model_type="logit", k=k, year_selections=year_selections
)
model_0_logit_out.to_csv("model_0_logit_metrics.csv", index=False)

model_0_rf_out = model_0_validation(
    model_type="rf", k=k, year_selections=year_selections
)
model_0_rf_out.to_csv("model_0_rf_metrics.csv", index=False)

# Model 1: 2 acting categories instead of 4
model_1_logit_out = model_1_validation(
    model_type="logit", k=k, year_selections=year_selections
)
model_1_logit_out.to_csv("model_1_logit_metrics.csv", index=False)

model_1_rf_out = model_1_validation(
    model_type="rf", k=k, year_selections=year_selections
)
model_1_rf_out.to_csv("model_1_rf_metrics.csv", index=False)

# Model 2: categorical variables instead of dummies
model_2_rf_out = model_0_validation(
    model_type="rf", k=k, year_selections=year_selections, predictor_set="model_2"
)
