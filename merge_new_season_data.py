...
def merge_new_into_old(new_season, data_folder='data/', ):
    # Load old data
    ...
    if df_acting_new['Winner'].isna().any() or \
            df_picture_new['Winner'].isna().any() or \
            df_director_new['Winner'].isna().any():
    ...
    df_picture.to_csv(data_folder + 'oscardata_picture.csv', index=False)
    print(df_picture.shape[0]-df_picture_old.shape[0], 'new rows added')
    df_director.to_csv(data_folder + 'oscardata_director.csv', index=False)
